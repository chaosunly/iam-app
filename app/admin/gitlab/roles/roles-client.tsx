...
interface Props {
  initialGroups: GitlabGroup[];
  initialProjects: GitlabProject[];
  initialIdentities: Identity[];
}

export function GitlabRolesClient({ initialGroups, initialProjects, initialIdentities }: Props) {
  ...
  const resources = resourceType === "group" ? initialGroups : initialProjects;
  ...
  function identityLabel(id: string): string {
    const identity = initialIdentities.find((i) => i.id === id);
  ...
  return (
    <div className="space-y-6 p-6 md:p-8">
          ...
          <form onSubmit={handleAssignRole} className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1.5">
                ...
                <Select value={resourceType} onValueChange={(v) => setResourceType(v as "group" | "project")}>
            ...
            <Button type="submit" disabled={loading}>{loading ? "Assigning..." : "Assign Role"}</Button>
          </form>
        </CardContent>
      </Card>
      ...
      {resourceId && (
        <Card>
          <CardHeader><CardTitle>Current Members ({members.length})</CardTitle></CardHeader>
          <CardContent>
            {members.length > 0 ? (
              <Table>
                <TableHeader>
          ...
          <dl className="space-y-2.5 text-sm">
            {GITLAB_ROLES.map((r) => (
              <div key={r} className="flex gap-3 items-baseline">
                <dt><Badge className={ROLE_BADGE_CLASSES[r]}>{r.charAt(0).toUpperCase() + r.slice(1)}</Badge></dt>
